    ...
    const onSubmitHandler = async (e) => {
        try {
            ...
            if (data.success) {
                ...
                setFiles([])
                await fetchProducts()
                navigate('/seller/product-list'); // move navigation here
            } else {
        ...
        <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col
         justify-between">
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                        ...
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className="max-w-24 cursor-pointer"
                                    src={
                                        files[index]
                                            ? files[index] instanceof File           // If user uploaded new file
                                                ? URL.createObjectURL(files[index])  // File → createObjectURL
                                                : files[index].preview               // Existing image from DB → use URL
                                            : assets.upload_area}               // No image
                ...
                <button
                    className="cursor-pointer px-8 py-2.5 bg-primary text-white font-medium rounded">Add</button>
            </form>
        </div>
    );
};
