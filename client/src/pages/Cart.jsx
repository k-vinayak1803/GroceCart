...
const Cart = () => {
    ...
    const { user, products, currency, setCartItems, cartItems, getCartCount, removeFromCart,
        updateCartItem, getCartAmount, navigate, axios } = useAppContext()
        ...
        try {
            ...
            if (paymentOption === "COD") {
                ...
                if (data.success && data.url) {
                    ...
                    window.location.href = data.url; // redirect safely to Stripe
                } else {
                    toast.error(data.message)
                }
            }
  ...
  if (orderId) {
    axios.post('/api/order/confirm', { orderId })
      .then(res => {
        if (res.data.success) {
          toast.success('Payment successful!');
          setCartItems({}); // ✅ clear only now
          navigate('/my-orders');
        } else {
